...
fn new_task(state: &AppState, req: NewTask) -> Result<Task> {
...
fn chat_dir(state: &AppState) -> Result<PathBuf> {
...
fn write_chat_context(state: &AppState, dir: &Path) -> Result<()> {
    let cfg = state.config.read();
    let mut md = String::from(
        "# Villain Layer — session context\n\n         Regenerated by Villain Layer every time a chat agent starts. You are in a          scratch directory, not a repository; use it freely for notes and drafts.\n\n",
    );
    ...
    md.push_str(
        "\nThose credentials live in the app, not in your environment. You can reach          these services only through MCP servers configured for your own CLI. If a          tool you need is missing, say so rather than guessing.\n\n",
    );
...
fn jira_client(state: &AppState) -> Result<(Jira, JiraConfig)> {
    let cfg = state
        .config
        .read()
        .jira
